...
HSPhy_NextGen_SchoolPCSum<-HSPhy_NextGen_SchoolIT301DF%>%
  ...
  select(`School Name`, `School Code`, `Practice Category`, `School Practice Cat%`)%>%
  pivot_wider(names_from = `Practice Category`, values_from = `School Practice Cat%`)%>%
  mutate(`MD%` =`B. Mathematics and Data`) %>%
  mutate(`IQ%` = `A. Investigations and Questioning`)%>%
  mutate(`ERM%` =`C. Evidence, Reasoning, and Modeling`)%>%
  mutate(`None%` = `None`)%>%
  mutate(`Subject` = "PHY")%>%
  select(`Subject`, `School Name`, `School Code`, `IQ%`, `MD%`, `ERM%`, `None%`)

HSPhy_NextGen_SchoolPCSum
