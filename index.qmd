...
advantage" and warnings differences among students are largely and "sometimes completely
...
```
::: blue
-   A school's percentage of student's classified as `Exceeding` expectations on the
    Introductory Physics MCAS is negatively associated with a school's variance in
    performance relative to student's in the state on `Mathematics and Data` items, `SD MD Diff`.

-   A school's summary performance on items in a given content `Reporting Category` as measured by `MF%`, `EN%`,      `WA%`, is positively associated with the `Reporting Category's` weight within the exam.
:::
...
across categories. All school's seem to perform the strongest on `Motion and Forces` items, followed by `Energy`, and weakest on `Waves` items. Notably, this is also the order of the relative weights of the content areas within the exam; `MF`, `EN`, and `WA` items account for **50%**, **30%**, and **20%** of exam points respectively.
